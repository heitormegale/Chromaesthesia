...
class Sound(threading.Thread):
    ...
    def run(self):
        ...
        #self.sample1 = 20*( signal.sawtooth(2 * np.pi * int(self.f)*(t+1),0.5)*t/(2.7)**2*np.e**(-t**2/(2*0.3**2)))
        #self.sample1 = ( signal.sawtooth(2 * np.pi * 440*(self.t)))
        self.sample1 =1/2-1/np.pi*(-np.sin(2*np.pi*self.f*self.t)+np.sin(4*np.pi*self.f*self.t)/2-np.sin(6*np.pi*self.f*self.t)/3)*np.e**(-50*(self.t)**7+1)/3
        #self.sample1= (np.sin(2 * np.pi * 440*self.t))
...
def Overlapsong(k,directory):
    ...
    file_1.export(os.path.join(directory,'IndividualSounds\overlap000' +str(k)+'.wav'), format='wav')
...
#sound(color1, color2, color3, 0, r"C:\Users\heito\Desktop\UCSB\Spring 2019\15C\Music")
