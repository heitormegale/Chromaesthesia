...
def Finalsong(directory):
    os.chdir(os.path.join(directory, "IndividualSounds"))
    infiles = []

    for root, dirs, files in os.walk(os.path.join(directory,"IndividualSounds")):  
        for filename in files:
            infiles.append(filename)
    print(infiles)    
    #infiles = ["overlap1.wav", "overlap2.wav","overlap3.wav"]
...
#sound2 = AudioSegment.from_wav("/path/to/file2.wav")
    ...
    combined_sounds.export(os.path.join(directory,"final_2.wav"), format="wav")
