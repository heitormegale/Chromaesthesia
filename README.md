...
This is a python project where we convert colors from an image to a melody. Our objective was to simulate a type of synestesia, where different senses activate eac other. In our case we would be creating a reverse Chromaestesia.
...
The code can be used independently of the scanner and it can analyze pictures in your computer or an image from your webcamera.


Since the maping of sounds to color in persons with Chrmaestesia is very unique and doesn't follow a pattern across the bearings of this condition, our task to reverse this process isn't dictated by any defined translation. Thus we created multiple methods, each with its downsides and upsides, that we will describe later.
